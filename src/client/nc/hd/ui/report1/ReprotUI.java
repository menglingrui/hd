...
//		buf.append(" e.dispname ct_code,");//科目名称----合同号
//		buf.append(" h.prepareddatev djrq,");//单据日期
//		buf.append(" d.pk_jobmngfil projectid, ");//项目
		ReportBaseVO[] vos7 = list.get(6);//报销单
		ReportBaseVO[] vos8 = (ReportBaseVO[]) CombinVO.combinData(vos7,
				new String[] { "ct_code","projectid","pk_currtype" }, new String[] { "yhwj", "xhwj",
						"bxhtfk", "num111", "byhtfk", "num112", "num22",
						"num23", "num33", "num34" }, ReportBaseVO.class);
		
		ReportBaseVO[] vos9 = list.get(7);//报销单
		...
		ReportBaseVO[] avos5= CombinVO.comin(avos4, vos8);//合并报销单
		ReportBaseVO[] avos6= CombinVO.comin(avos5, vos10);//合并报销单
	 ...
	 * @说明：完达山物流项目
	 * @时间：2012-9-5上午10:06:38
	 * @return xyh是合同主表pk,cksqsh是合同附表pk
	 * @throws Exception
	 */
	private String getSql3() throws Exception {
		...
		sql.append(" b.jfybje bxhtfk,");// 付款原币金额
		sql.append(" b.jfbbje num111, ");// 付款本币金额
	...
	private String getSql4() throws Exception {
		...
		sql.append(" h.djbh ,");// 单据号
		sql.append(" h.djrq ,");// 单据日期
		sql.append(" b.jfybje byhtfk,");// 付款原币金额
		sql.append(" b.jfbbje num112, ");// 付款本币金额
		sql.append(" b.cksqsh pk_ct_manage_b ");// 上层来源单据行id
		sql.append(" from arap_djzb h ");
		sql.append(" join arap_djfb b on h.vouchid =b.vouchid  ");
		sql.append(" join ct_manage_b c on b.cksqsh=c.pk_ct_manage_b ");
	...
	private String getSql5() throws Exception {
		...
		sql.append(" h.djbh ,");// 单据号
		sql.append(" h.djrq ,");// 单据日期
		sql.append(" b.jfybje num22,");// 付款原币金额
		sql.append(" b.jfbbje num23, ");// 付款本币金额
		sql.append(" b.cksqsh pk_ct_manage_b ");// 上层来源单据行id
		sql.append(" from arap_djzb h ");
		sql.append(" join arap_djfb b on h.vouchid =b.vouchid  ");
		sql.append(" join ct_manage_b c on b.cksqsh=c.pk_ct_manage_b ");
	...
	private String getSql6() throws Exception {
		...
		sql.append(" h.djbh ,");// 单据号
		sql.append(" h.djrq ,");// 单据日期
		sql.append(" b.jfybje num33,");// 付款原币金额
		sql.append(" b.jfbbje num34, ");// 付款本币金额
		sql.append(" b.cksqsh pk_ct_manage_b ");// 上层来源单据行id
		sql.append(" from arap_djzb h ");
		sql.append(" join arap_djfb b on h.vouchid =b.vouchid  ");
		sql.append(" join ct_manage_b c on b.cksqsh=c.pk_ct_manage_b ");
...
//		StringBuffer sql = new StringBuffer();
//		sql.append(" select h.pk_corp , ");// 公司
//		sql.append(" h.djbh  ct_code,");//报销单号
//		sql.append(" h.bzbm pk_currtype , ");// 币种
//		sql.append(" h.djrq    ,");// 单据日期
//		sql.append(" b.jobid projectid ,");// 项目
//		sql.append(" b.ybje xhwj ,");// 原币金额
//		sql.append(" b.bbje ");// 本币金额
...
//		sql.append(" b.bbje ");// 本币金额
		...
		buf.append(" h.prepareddatev djrq,");//单据日期
		buf.append(" d.pk_jobmngfil projectid, ");//项目
		buf.append(" h.pk_currtype pk_currtype,");//币种
		buf.append(" h.debitamount  xhwj, ");//原币借方金额
		buf.append(" h.localdebitamount bbje,");//本币借方金额
		buf.append(" h.debitamount bxhtfk,");// 付款原币金额
		buf.append(" h.localdebitamount num111, ");// 付款本币金额
		buf.append(" h.debitamount num33,");// 累计付款原币金额
		buf.append(" h.localdebitamount num34 ");// 累计付款本币金额
...
//		buf.append(" b.checkvalue");//部门档案的主键
		...
		buf.append(" and h.direction='D'");//借贷 方向为借方
...
//		sql.append(" b.bbje ");// 本币金额
		...
		buf.append(" d.pk_jobmngfil projectid, ");//项目
		buf.append(" h.pk_currtype pk_currtype,");//币种
		buf.append(" h.debitamount  yhwj, ");//原币借方金额
		buf.append(" h.localdebitamount bbje,");//本币借方金额
//		buf.append(" h.debitamount byhtfk,");// 付款原币金额
//		buf.append(" h.localdebitamount num112, ");// 付款本币金额
		buf.append(" h.debitamount num22,");// 累计付款原币金额
		buf.append(" h.localdebitamount num23 ");// 累计付款本币金额
	//	buf.append(" h.localcreditamount num2,");//贷  流出资金
//		buf.append(" c.def2 num20,");//期初流入资金
//		buf.append(" c.def17  num21,");//期初流出资金
//		buf.append(" c.def18 num22,");//期初存款利息
